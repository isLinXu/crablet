use anyhow::Result;
use crablet as sqlx;
use crablet::memory::semantic::{KnowledgeGraph, SqliteKnowledgeGraph};
...
async fn test_demo_f_rag_knowledge_graph() -> Result<()> {
    ...
    kg.add_relation("Crablet", "High Performance", "has_feature")
        .await?;
    ...
    assert!(relations
        .iter()
        .any(|(_, rel, target)| rel == "written_in" && target == "Rust"));
    assert!(relations
        .iter()
        .any(|(_, rel, target)| rel == "has_feature" && target == "High Performance"));
