...
async fn test_episodic_memory_sqlite() {
    ...
    let mem = EpisodicMemory::new(db_url)
        .await
    ...
    mem.save_message(session_id, "user", "Hello history")
        .await
        .expect("Save failed");
    ...
    // 3. Add more
    mem.save_message_transactional(session_id, "assistant", "Hi back")
        .await
        .expect("Save failed");
...
#[cfg(feature = "knowledge")]
#[tokio::test]
async fn test_episodic_memory_persists_message_metrics() {
    use crablet as sqlx;
    use sqlx::Row;

    let mem = EpisodicMemory::new("sqlite::memory:")
        .await
        .expect("Failed to create episodic memory");
    ...
    .await
    .expect("Save with metrics failed");

    let row = sqlx::query(
        "SELECT m.tokens, m.latency_ms, s.message_count
         FROM messages m
         JOIN sessions s ON s.id = m.session_id
         WHERE m.session_id = ?",
    )
    .bind("metrics-session")
    .fetch_one(&mem.pool)
    .await
    .expect("metrics row should exist");

    assert_eq!(row.get::<Option<i64>, _>("tokens"), Some(42));
    assert_eq!(row.get::<Option<i64>, _>("latency_ms"), Some(1234));
    assert_eq!(row.get::<i64, _>("message_count"), 1);
}
