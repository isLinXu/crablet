...
use crablet::cognitive::{
    create_llm_client, ExecutionRequest, ExecutionResult, MetaCognitiveController, MetaConfig,
};
...
async fn test_feedback_integration() {
    ...
    // Integrate feedback
    controller
        .integrate_feedback("feedback-test", 0.9)
        .await
        .unwrap();
...
async fn test_meta_cognitive_workflow() {
    ...
    controller
        .execute_with_meta(request1, |req| ExecutionResult {
            task_id: req.task_id.clone(),
            success: false,
            output: "Failed".into(),
            confidence: 0.2,
    ...
    controller
        .execute_with_meta(request2, |req| ExecutionResult {
            task_id: req.task_id.clone(),
            success: true,
            output: "Success".into(),
            confidence: 0.95,
    ...
    println!(
        "Workflow test - Patterns extracted: {}",
