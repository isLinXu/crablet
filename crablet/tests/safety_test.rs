use crablet::safety::oracle::{SafetyDecision, SafetyLevel, SafetyOracle};
use std::fs::File;
...
fn test_safety_oracle_basic() {
    ...
    assert_eq!(
        oracle.check_bash_command("echo hello"),
        SafetyDecision::Allowed
    );
    assert_eq!(
        oracle.check_bash_command("grep search file.txt"),
        SafetyDecision::Allowed
    );
    ...
    match decision {
        ...
        _ => panic!(
            "Expected RequireConfirmation for 'netcat', got {:?}",
            decision
        ),
    }
    ...
    match decision {
        SafetyDecision::RequireConfirmation(_) => {}
        _ => panic!(
            "Expected RequireConfirmation for chained 'rm', got {:?}",
            decision
        ),
    }
}
...
fn test_safety_oracle_file_access() {
    ...
    let oracle =
        SafetyOracle::new(SafetyLevel::Strict).with_allowed_directories(vec![dir_path.clone()]);

    // 1. Safe Access
    assert_eq!(
        oracle.check_file_access(file_path.to_str().unwrap()),
        SafetyDecision::Allowed
    );
    assert_eq!(
        oracle.check_file_access(nested_file.to_str().unwrap()),
        SafetyDecision::Allowed
    );
    ...
    assert_eq!(
        oracle.check_file_access(&traversal_safe),
        SafetyDecision::Allowed
    );
    ...
    assert_eq!(
        oracle.check_file_access(non_existent.to_str().unwrap()),
        SafetyDecision::Allowed
    );
    ...
    match oracle.check_file_access(non_existent_out.to_str().unwrap()) {
        SafetyDecision::Blocked(_) => {}
        d => panic!(
            "Expected Blocked for non-existent outside file, got {:?}",
            d
        ),
    }
}
