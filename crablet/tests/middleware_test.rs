...
use crablet::cognitive::middleware::{
    CognitiveMiddleware, CostGuardMiddleware, MiddlewareState, SafetyMiddleware,
};
...
impl crablet::cognitive::llm::LlmClient for MockLlm {
    async fn chat_complete(&self, _: &[Message]) -> Result<String> {
        Ok("mock".to_string())
    }
...
async fn test_safety_middleware() -> Result<()> {
    ...
    let result = middleware
        .execute("Hello world", &mut context, &state)
        .await?;
    ...
    let result = middleware
        .execute(&long_input, &mut context, &state)
        .await?;
    assert!(result.is_some());
    let (response, _) = result.unwrap();
...
async fn test_cost_guard_middleware() -> Result<()> {
    ...
    assert_eq!(
        context.len(),
        6,
        "Context should be truncated to 6 messages, got {}",
        context.len()
    );
