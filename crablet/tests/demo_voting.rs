use anyhow::Result;
use crablet::agent::swarm::{AgentId, Swarm, SwarmAgent, SwarmMessage};
...
impl SwarmAgent for MockVoter {
    ...
    async fn receive(&mut self, message: SwarmMessage, _sender: AgentId) -> Option<SwarmMessage> {
        match message {
            SwarmMessage::Task { task_id, .. } => Some(SwarmMessage::Result {
                task_id,
...
async fn test_voting_flow() -> Result<()> {
    ...
    // 3 Voters: 2 Yes, 1 No
    let v1 = MockVoter {
        id: AgentId::from_name("V1"),
        name: "V1".to_string(),
        vote: "YES, I agree".to_string(),
    };
    let v2 = MockVoter {
        id: AgentId::from_name("V2"),
        name: "V2".to_string(),
        vote: "YES, sounds good".to_string(),
    };
    let v3 = MockVoter {
        id: AgentId::from_name("V3"),
        name: "V3".to_string(),
        vote: "NO, risky".to_string(),
    };
    ...
    swarm
        .register_agent(Box::new(UserAgent {
            id: user_id.clone(),
            tx,
        }))
        .await;
    ...
    swarm
        .send(&AgentId::from_name("Chairperson"), start_msg, &user_id)
        .await?;
