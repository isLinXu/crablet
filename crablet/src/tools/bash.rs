...
impl BashTool {
    fn validate_command(cmd: &str) -> Result<()> {
        ...
        let allowed_binaries = [
            "ls", "grep", "cat", "echo", "pwd", "whoami", "date", "git", "cargo", "mkdir", "touch",
            "rm", "cp", "mv", "python", "python3", "node", "npm", "tree", "find", "head", "tail",
            "wc", "sort", "uniq", "awk", "sed", "ps",
        ];
        ...
        if !allowed_binaries.contains(&binary_name) {
            return Err(anyhow!(
                "Command binary '{}' is not in the whitelist",
                binary_name
            ));
        }
    ...
    pub async fn execute(cmd: &str) -> Result<String> {
        ...
        let executor = DockerExecutor::strict().with_timeout(10); // 10 second timeout

        info!("Executing bash command in Docker sandbox: {}", cmd);

        // Execute command in Docker sandbox
        let result = executor
            .execute("alpine:latest", &["sh", "-c", cmd])
            .await?;
...
mod tests {
    ...
    #[test]
    fn validate_command_blocks_shell_operators() {
        let error = BashTool::validate_command("ls | grep foo").unwrap_err();
        assert!(error.to_string().contains("dangerous characters"));
    }

    #[test]
    fn validate_command_blocks_unknown_binary() {
        let error = BashTool::validate_command("nmap 127.0.0.1").unwrap_err();
        assert!(error.to_string().contains("not in the whitelist"));
    }

    #[tokio::test]
    async fn plugin_reports_confirmation_before_static_operator_block() {
        let plugin = BashPlugin::new(SafetyOracle::new(SafetyLevel::Strict));
        let output = plugin
            .execute("run", json!({"cmd": "ls ; rm file"}))
            .await
            .unwrap();
