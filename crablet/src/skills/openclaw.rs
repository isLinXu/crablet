use crate::skills::{Skill, SkillDependencies, SkillManifest, SkillResources, SkillTrigger};
...
impl OpenClawSkillLoader {
    pub async fn load(path: &Path) -> Result<Skill> {
        ...
        let manifest = SkillManifest {
            ...
            parameters: frontmatter.parameters.unwrap_or_else(|| {
                serde_json::json!({
                    "type": "object",
                    "properties": {},
        ...
        Ok(Skill {
            ...
            path: path
                .parent()
                .ok_or_else(|| anyhow!("Invalid skill path: has no parent"))?
                .to_path_buf(),
        })
    }
    ...
    fn generate_triggers(frontmatter: &FrontMatter) -> Vec<SkillTrigger> {
        ...
        let keywords: Vec<String> = frontmatter
            .description
            .split_whitespace()
            .filter(|w| w.len() > 3)
            .take(5)
