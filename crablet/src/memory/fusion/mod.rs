...
impl FusionMemorySystem {
    ...
    pub async fn initialize(config: Arc<FusionConfig>) -> Result<Self, MemoryError> {
        ...
        let daily_logs =
            Arc::new(daily_logs::DailyLogs::from_config(&config.memory.daily_logs).await?);
        debug!("Daily Logs initialized");

        // Initialize Memory Weaver
        let weaver = Arc::new(weaver::MemoryWeaver::new(config.memory.semantic.clone()));
    ...
    pub async fn create_session(
        &self,
        session_id: String,
    ) -> Result<Arc<layer_session::SessionLayer>, MemoryError> {
        ...
        let system_message = self
            .build_context_message(&recent_logs, &relevant_memories)
            .await?;
    ...
    /// Get or create a session for the provided session ID.
    ...
    pub async fn export_to_markdown(&self, workspace_path: &Path) -> Result<(), MemoryError> {
        ...
        let user_guard = self.user.read().await;
        user_guard
            .export_to_markdown(&workspace_path.join("USER.md"))
            .await?;
        drop(user_guard);
        ...
        let user_guard = self.user.read().await;
        user_guard
            .export_memories_to_markdown(&workspace_path.join("MEMORY.md"))
            .await?;
        drop(user_guard);
