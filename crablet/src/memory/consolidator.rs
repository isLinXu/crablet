...
impl MemoryConsolidator {
    ...
    pub fn start_background_loop(self: Arc<Self>, memory: Arc<EpisodicMemory>) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // Remove duplicates
                let unique_sessions: Vec<String> = sessions
                    .into_iter()
                    .collect::<HashSet<_>>()
                    .into_iter()
                    .collect();
    ...
    pub async fn on_message_added(&self, session_id: &str) {
        ...
        *count += 1;
        ...
        if *count >= 20 {
            info!(
                "Auto-triggering memory consolidation for session {} (message count threshold)",
                session_id
            );
            *count = 0; // Reset
    ...
    pub async fn check_time_based_trigger(&self, session_id: &str) {
        ...
        if now.duration_since(*last_time) > Duration::from_secs(3600) {
            // 1 hour
            info!(
                "Auto-triggering memory consolidation for session {} (time threshold)",
                session_id
            );
