    ...
    fn chunk(&self, text: &str) -> Result<Vec<Chunk>, CrabletError> {
        ...
        for line in text.lines() {
            if let Some((level, title)) = Self::is_header(line) {
                ...
                // Check size limit
                if current_chunk.len() + line.len() > self.chunk_size
...
impl RecursiveCharacterChunker {
        ...
        for split in splits {
                ...
                && !current_doc.is_empty()
            {
                ...
                while total > self.chunk_overlap
                    || (total + len + separator_len > self.chunk_size && total > 0)
                {
                    if !current_doc.is_empty() {
    ...
    fn _chunk(&self, text: &str, separators: &[String]) -> Vec<String> {
        ...
        let Some(last_separator) = separators.last() else {
            return vec![text.to_string()];
        };
        let mut separator = last_separator.as_str();
