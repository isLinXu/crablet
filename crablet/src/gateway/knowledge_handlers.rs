...
pub async fn list_documents(State(gateway): State<Arc<CrabletGateway>>) -> Json<serde_json::Value> {
    let _ = gateway;
...
pub async fn upload_knowledge(
    ...
    let _ = multipart;
    let allowed = [
        "pdf", "doc", "docx", "txt", "md", "csv", "xls", "xlsx", "jpg", "jpeg", "png", "gif",
        "svg", "webp", "mp3", "wav", "m4a", "flac", "mp4", "avi", "mov", "mkv",
    ];
    ...
    if let Some(ingestion) = &gateway.ingestion {
            ...
            .map_err(|_| StatusCode::BAD_REQUEST)?
        {
            ...
            if data.len() > max_size {
                tracing::warn!(
                    "Blocked oversize upload: {} bytes for {}",
                    data.len(),
                    file_name
                );
