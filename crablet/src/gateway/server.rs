use crate::agent::distributed_harness::DistributedHarnessManager;
...
use crate::gateway::chat_enhancement_handlers::{
    dual_search, get_star_count, get_token_usage, is_starred, list_stars, star_message,
    topk_recommend, unstar_message,
};
use crate::gateway::knowledge_handlers::{
    get_document_chunks, list_documents, search_knowledge, upload_knowledge,
};
...
use crate::gateway::routing_handlers::{
    create_api_key, get_routing_report, get_routing_settings, get_system_config, list_api_keys,
    revoke_api_key, update_routing_settings, update_system_config,
};
use crate::gateway::rpc::RpcDispatcher;
use crate::gateway::rpc_handlers::rpc_handler;
use crate::gateway::session::SessionManager;
use crate::gateway::session_handlers::{
    compress_session, delete_session as delete_chat_session,
    get_session_history as get_chat_session_history, list_sessions as list_chat_sessions,
};
use crate::gateway::types::GatewayConfig;
use crate::gateway::web_handlers::{
    batch_test_skills, cancel_task, chat_handler, chat_stream, decide_swarm_review,
    get_all_skill_logs, get_dashboard_stats, get_mcp_overview, get_skill_logs, get_skills_sh_top,
    get_swarm_state, get_swarm_stats, get_swarm_tasks, image_handler, install_skill, list_agents,
    list_skills, list_swarm_reviews, run_skill, search_registry_skills, semantic_search_skills,
    toggle_skill,
};
...
pub struct CrabletGateway {
    ...
    pub distributed_harness: Option<Arc<DistributedHarnessManager>>,
...
impl CrabletGateway {
    ...
    pub async fn new(
        ...
        #[cfg(feature = "knowledge")]
        let ingestion = router
            .sys2
            .vector_store
            .as_ref()
            .map(|vs| Arc::new(IngestionService::new(vs.clone())));
    ...
    pub fn with_distributed_harness(
        mut self,
        distributed_harness: Arc<DistributedHarnessManager>,
    ) -> Self {
        self.distributed_harness = Some(distributed_harness);
        self
    }
    ...
    fn build_router(gateway: Arc<Self>) -> Router {
        ...
        let legacy_api_routes = Router::new()
            ...
            .route(
                "/api/swarm/reviews/:task_id/decision",
                post(decide_swarm_review),
            )
