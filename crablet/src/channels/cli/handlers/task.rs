...
use crate::auto_working::{
    ScheduleType, ScheduledTask, SqliteQueue, SqliteTaskStore, TaskAction, TaskStore,
};
...
pub async fn handle_task(subcmd: &TaskSubcommands, config: &Config) -> Result<()> {
    ...
    match subcmd {
        TaskSubcommands::List { status } => list_tasks(&store, status.as_deref()).await,
        TaskSubcommands::Schedule {
            name,
            cron,
            at,
            action_type,
        ...
        TaskSubcommands::Cancel { id } => cancel_task(&store, id).await,
        TaskSubcommands::Show { id } => show_task(&store, id).await,
        TaskSubcommands::History { id, limit } => show_history(&queue, id.as_deref(), *limit).await,
        TaskSubcommands::Pause { id } => pause_task(&store, id).await,
        TaskSubcommands::Resume { id } => resume_task(&store, id).await,
        TaskSubcommands::Run { name, args } => run_task(name, args).await,
        TaskSubcommands::Queue {
            pending,
            running,
            failed,
...
async fn list_tasks(store: &SqliteTaskStore, status_filter: Option<&str>) -> Result<()> {
    ...
    for task in tasks {
        ...
        let status_colored = if task.enabled {
            "active".green()
        } else {
            "paused".yellow()
        };
...
async fn schedule_task(
    ...
    let action = match action_type {
        ...
        _ => anyhow::bail!(
            "Unknown action type: {}. Supported: cognitive, workflow",
            action_type
        ),
    };
...
async fn show_history(queue: &SqliteQueue, task_id: Option<&str>, limit: usize) -> Result<()> {
    ...
    println!(
        "Note: Loading last {} task execution records from queue...",
        limit
    );
    println!();
...
async fn pause_task(store: &SqliteTaskStore, id: &str) -> Result<()> {
    ...
    println!("{} Task '{}' paused", "✓".green().bold(), id.cyan());
...
async fn resume_task(store: &SqliteTaskStore, id: &str) -> Result<()> {
    ...
    println!("{} Task '{}' resumed", "✓".green().bold(), id.cyan());
...
async fn show_queue(queue: &SqliteQueue, pending: bool, running: bool, failed: bool) -> Result<()> {
    ...
    if pending || (!running && !failed) {
        ...
        for task in pending_tasks {
            println!(
                "  - [{}] {} (Priority: {})",
                task.id, task.payload.task_type, task.priority
            );
        }
    }
