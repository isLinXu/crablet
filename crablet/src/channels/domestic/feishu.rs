...
impl FeishuChannel {
    ...
    async fn get_tenant_access_token(&self) -> Result<String> {
        ...
        let app_id = self
            .app_id
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("App ID missing"))?;
        let app_secret = self
            .app_secret
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("App Secret missing"))?;
        ...
        let token = body["tenant_access_token"]
            .as_str()
            .ok_or(anyhow::anyhow!("No tenant_access_token"))?
            .to_string();
...
impl Channel for FeishuChannel {
    ...
    async fn start(&self) -> Result<()> {
        ...
        // Event subscription server (WebHook Handler):
        // This requires Axum integration which is in the gateway module.
        // The gateway exposes a `/channels/feishu/webhook` endpoint that
        // receives event callbacks from the Feishu platform. This channel
        // provides the handler function that the gateway calls when events
        // arrive. Full event subscription will be added when the gateway
