...
use crate::connectors::{
    CalendarEventType, Connector, ConnectorConfig, ConnectorError, ConnectorEvent, ConnectorHealth,
    ConnectorResult, HealthStatus,
};
...
impl CalendarConnector {
    pub fn new(config: ConnectorConfig) -> ConnectorResult<Self> {
        let calendar_config: CalendarConfig = serde_json::from_value(config.settings.clone())
            .map_err(|e| {
                ConnectorError::ConfigurationError(format!("Invalid calendar config: {}", e))
            })?;
                ...
                match Self::fetch_events(&lookahead_time).await {
                    Ok(events) => {
                        for event in events {
                            ...
                            {
                                let triggered =
                                    triggered_events.entry(event.id.clone()).or_default();
                                if !triggered.contains(&CalendarEventType::EventStarted) {
                                    triggered.push(CalendarEventType::EventStarted);
                                    if event_types.is_empty()
                                        || event_types.contains(&CalendarEventType::EventStarted)
                                    {
                                        Self::emit_event(
                                            &event_tx,
                                            &event,
                                            CalendarEventType::EventStarted,
                                        )
                                        .await;
                                    }
                                }
                            ...
                            if time_until_end.num_minutes() <= 5 && time_until_end.num_seconds() > 0
                            {
                                ...
                                if !triggered.contains(&CalendarEventType::EventEnded) {
                                    ...
                                    if event_types.is_empty()
                                        || event_types.contains(&CalendarEventType::EventEnded)
                                    {
                                        Self::emit_event(
                                            &event_tx,
                                            &event,
                                            CalendarEventType::EventEnded,
                                        )
                                        .await;
                                    }
                                }
                            ...
                            for reminder in &event.reminders {
                                let reminder_time =
                                    event.start_time - Duration::minutes(reminder.minutes_before);
                                    ...
                                    && time_until_reminder.num_seconds() > -60
                                {
                                    ...
                                    if !triggered.contains(&CalendarEventType::EventReminder) {
                                        ...
                                        if event_types.is_empty()
                                            || event_types
                                                .contains(&CalendarEventType::EventReminder)
                                        {
                                            Self::emit_event(
                                                &event_tx,
                                                &event,
                                                CalendarEventType::EventReminder,
                                            )
                                            .await;
                                        }
                                    }
    ...
    async fn fetch_events(
        _lookahead_time: &DateTime<Utc>,
    ) -> ConnectorResult<Vec<CalendarEventData>> {
...
impl Connector for CalendarConnector {
    ...
    async fn connect(&mut self) -> ConnectorResult<()> {
        info!(
            ...
            self.calendar_config.provider
        );

        // Validate configuration
        match self.calendar_config.provider {
            CalendarProvider::Google | CalendarProvider::Outlook => {
                if self.calendar_config.credentials_path.is_none()
                    && self.calendar_config.access_token.is_none()
                {
