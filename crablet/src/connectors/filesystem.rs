...
use crate::connectors::{
    ...
    FileChangeType, FileMetadata, HealthStatus,
};
    ...
    pub fn new(config: ConnectorConfig) -> ConnectorResult<Self> {
        let fs_config: FileSystemConfig =
            serde_json::from_value(config.settings.clone()).map_err(|e| {
                ConnectorError::ConfigurationError(format!("Invalid filesystem config: {}", e))
            })?;
...
impl Connector for FileSystemConnector {
    ...
    async fn start(&mut self) -> ConnectorResult<()> {
        ...
        info!(
            "Filesystem connector '{}' started watching {:?}",
            self.config.name, self.fs_config.paths
        );
        Ok(())
    }
