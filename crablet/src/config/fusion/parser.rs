...
impl MarkdownDocument {
    ...
    pub fn parse(content: &str, path: PathBuf) -> Result<Self, ParseError> {
        ...
        let frontmatter_re = Regex::new(r"^---\s*\n(.*?)\n---\s*\n(.*)$")
            .map_err(|e| ParseError::InvalidFormat(format!("Regex compilation failed: {e}")))?;
    ...
    pub fn parse_frontmatter<T: serde::de::DeserializeOwned>(
        ...
        match &self.frontmatter {
            Some(fm) => {
                let value: T =
                    serde_yaml::from_str(fm).map_err(|e| ParseError::YamlError(e.to_string()))?;
                Ok(Some(value))
            }
            None => Ok(None),
        }
...
impl AgentsParser {
    ...
    fn parse_identity(
        ...
        let name_re = Regex::new(r"\*\*名称\*\*[:：]\s*(.+)|\*\*Name\*\*[:：]\s*(.+)")
            .map_err(|e| ParseError::InvalidFormat(format!("Regex compilation failed: {e}")))?;
        let name = name_re
            .captures(identity_section)
...
impl SoulParser {
    ...
    fn parse_personality(
        ...
        // Parse from content
        let personality_section = sections
            .get("人格特质")
        ...
        let values = if let Some(section) = values_section {
            ...
            let value_re =
                Regex::new(r"(\d+)\.\s*\*\*(.+?)\*\*[:：]?\s*(.*)").unwrap_or_else(|_| {
                    ...
                    Regex::new(r"(\d+)\.\s*\*\*(.+?)\*\*")
                        .expect("fallback value regex should compile")
                });
            value_re
    ...
    fn parse_principles(
        ...
        let principles_section = sections
            .get("不可变原则")
...
impl FusionConfigParser {
    ...
    pub async fn parse_workspace(workspace_path: &Path) -> Result<FusionConfig, ParseError> {
        ...
        let soul_content = tokio::fs::read_to_string(workspace_path.join("SOUL.md"))
            .await
            .map_err(|e| ParseError::IoError(e.to_string()))?;
        let user_content = tokio::fs::read_to_string(workspace_path.join("USER.md"))
            .await
            .map_err(|e| ParseError::IoError(e.to_string()))?;

        // Parse each document
        let agents_doc =
            MarkdownDocument::parse(&agents_content, workspace_path.join("AGENTS.md"))?;
