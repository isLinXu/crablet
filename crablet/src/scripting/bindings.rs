...
use crate::safety::oracle::{SafetyDecision, SafetyLevel, SafetyOracle};
use crate::tools::bash::BashTool;
use crate::tools::file::FileTool;
use crate::tools::search::WebSearchTool;
use crate::types::Message;
use mlua::{ExternalError, IntoLua, Lua, Result};
...
pub fn register_bindings(lua: &Lua) -> Result<()> {
    ...
    crablet.set(
        "run_command",
        lua.create_async_function(|_, cmd: String| async move {
            let oracle = SafetyOracle::new(SafetyLevel::Strict);
            match oracle.check_bash_command(&cmd) {
                SafetyDecision::Allowed => match BashTool::execute(&cmd).await {
                    Ok(output) => Ok(output),
                    Err(e) => Ok(format!("Error: {}", e)),
                },
                SafetyDecision::Blocked(reason) => {
                    Ok(format!("🚫 Safety Oracle Blocked: {}", reason))
                }
                SafetyDecision::RequireConfirmation(reason) => {
                    Ok(format!("⚠️ Confirmation Required: {}", reason))
                }
            }
        })?,
    )?;
    ...
    crablet.set(
        ...
        lua.create_async_function(|lua, path: String| async move {
            ...
            match oracle.check_file_access(&path) {
                ...
                SafetyDecision::Blocked(reason) => {
                    format!("🚫 Safety Oracle Blocked: {}", reason).into_lua(&lua)
                }
                SafetyDecision::RequireConfirmation(reason) => {
                    format!("⚠️ Confirmation Required: {}", reason).into_lua(&lua)
                }
            }
        })?,
    )?;
    ...
    crablet.set(
        "llm_chat",
        lua.create_async_function(|_, (model, prompt): (String, String)| async move {
    ...
    crablet.set(
        "audio_speak",
        lua.create_async_function(|_, (text, output_path): (String, String)| async move {
            let processor = AudioTool::new().map_err(|e| e.into_lua_err())?;
            processor
    ...
    crablet.set(
        "extract_knowledge",
        lua.create_async_function(|_, text: String| async move {
    ...
    crablet.set(
        "search",
        lua.create_async_function(|_, query: String| async move {
