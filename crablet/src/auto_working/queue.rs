...
impl SqliteQueue {
    ...
    pub async fn get_stats(&self) -> Result<QueueStats> {
        let pending: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM work_queue WHERE processing = 0")
                .fetch_one(&self.pool)
                .await?;
        let running: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM work_queue WHERE processing = 1")
                .fetch_one(&self.pool)
                .await?;
        let failed: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM dead_letter_queue")
            .fetch_one(&self.pool)
            .await?;
...
impl QueueBackend for SqliteQueue {
    ...
    async fn requeue(&self, task_id: &str, delay: Duration) -> Result<()> {
        let scheduled_at =
            Utc::now() + chrono::Duration::from_std(delay).unwrap_or(chrono::Duration::seconds(0));
...
mod tests {
    ...
    async fn test_sqlite_queue_basic() {
        let queue = SqliteQueue::new("sqlite::memory:")
            .await
            .expect("failed to create sqlite queue");
    ...
    async fn test_priority_ordering() {
        ...
        queue
            .enqueue(medium_priority)
            .await
            .expect("failed to enqueue medium priority task");

        // Dequeue should return highest priority first
        let first = queue
            .dequeue(Duration::from_secs(1))
            .await
            .expect("failed to dequeue first task")
            .expect("first dequeued task should be Some");
        assert_eq!(first.priority, 90);
        queue
            .ack(&first.id)
            .await
            .expect("failed to ack first task");

        let second = queue
            .dequeue(Duration::from_secs(1))
            .await
            .expect("failed to dequeue second task")
            .expect("second dequeued task should be Some");
        assert_eq!(second.priority, 50);
        queue
            .ack(&second.id)
            .await
            .expect("failed to ack second task");

        let third = queue
            .dequeue(Duration::from_secs(1))
            .await
            .expect("failed to dequeue third task")
            .expect("third dequeued task should be Some");
    ...
    async fn test_scheduled_task() {
        ...
        let immediate = queue
            ...
            .await
            .expect("failed to dequeue (should be none)");
        ...
        let later = queue
            .dequeue(Duration::from_secs(1))
            .await
            .expect("failed to dequeue later task");
    ...
    async fn test_dead_letter_queue() {
        ...
        queue
            .enqueue(task.clone())
            .await
            .expect("failed to enqueue task for dlq test");

        // Move to DLQ
        queue
            .move_to_dlq(&task, "Test error")
            .await
