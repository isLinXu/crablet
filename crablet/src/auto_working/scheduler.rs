                            ...
                            tokio::spawn(async move {
                                ...
                                match executor.execute(&task.action).await {
                                    Ok(result) => {
                                        ...
                                        let retry_count = if result.success {
                                            0
                                        } else {
                                            task.retry_count + 1
                                        };
                                        ...
                                        if let Err(e) = store
                                            .update_last_run(&task_id, Utc::now(), run_count)
                                            .await
                                        {
                                            error!(
                                                "Failed to update last_run for task {}: {}",
                                                task_id, e
                                            );
                                        }

                                        if let Err(e) =
                                            store.update_next_run(&task_id, next_run).await
                                        {
                                            error!(
                                                "Failed to update next_run for task {}: {}",
                                                task_id, e
                                            );
                                        }
                                        ...
                                        if retry_count != 0 {
                                            ...
                                            {
                                                error!(
                                                    "Failed to update retry_count for task {}: {}",
                                                    task_id, e
                                                );
                                            }
                                        }
                                    ...
                                    Err(e) => {
                                        ...
                                        if let Err(e) =
                                            store.update_retry_count(&task_id, retry_count).await
                                        {
                                            error!(
                                                "Failed to update retry_count for task {}: {}",
                                                task_id, e
...
mod tests {
    ...
    async fn test_sqlite_task_store() {
        ...
        store
            .update_next_run(&task.id, Some(next_run))
            .await
            .unwrap();
