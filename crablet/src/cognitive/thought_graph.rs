...
impl ThoughtGraph {
    ...
    pub fn get_stats(&self) -> ThoughtGraphStats {
        ...
        let total_duration: u64 = self.nodes.values().filter_map(|n| n.duration_ms).sum();
        ...
        ThoughtGraphStats {
            ...
            average_confidence: if avg_confidence > 0.0 {
                Some(avg_confidence)
            } else {
                None
            },
...
impl ThoughtGraphBuilder {
    ...
    pub fn add_decision(
        ...
        self.graph
            .connect_sequential(&self.current_node_id, &decision_id);
...
impl<'a> DecisionBranch<'a> {
    ...
    pub fn choose(&mut self, choice_index: usize) -> &mut ThoughtGraphBuilder {
        if choice_index < self.choices.len() {
            ...
            let branch_node =
                ThoughtNode::new(ThoughtNodeType::Reasoning, format!("分支: {}", choice))
                    .with_parent(self.decision_id.clone());

            let branch_id = self.builder.graph.add_node(branch_node);
            self.builder
                .graph
                .connect_branch(&self.decision_id, &branch_id, choice);
...
mod tests {
    ...
    fn test_node_creation() {
        let node = ThoughtNode::new(ThoughtNodeType::Reasoning, "测试推理").with_confidence(0.85);
