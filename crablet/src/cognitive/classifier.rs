...
impl Classifier {
    pub fn classify_intent(input: &str) -> Intent {
        ...
        if input_lower.starts_with("hi")
            || input_lower.starts_with("hello")
            || input_lower == "test"
            || input_lower.contains("你好")
            || input_lower.starts_with("嗨")
        {
        ...
        if input_lower.contains("status")
            || input_lower.contains("system info")
        ...
        if input_lower.contains("who are you")
            || input_lower.contains("what are you")
            || input_lower.contains("your name")
            || input_lower.contains("introduce yourself")
            || input_lower.contains("你是谁")
            || input_lower.contains("你是什么")
            || input_lower.contains("你叫什么")
            || input_lower.contains("介绍一下")
            || input_lower.contains("你是干嘛")
            || input_lower.contains("你是做什么")
            || input_lower.contains("你的身份")
            || input_lower.contains("你的角色")
            || input_lower.contains("你是ai")
            || input_lower.contains("你是人工智能")
            || input_lower.contains("谁创造了你")
            || input_lower.contains("谁开发了")
        {
        ...
        if input_lower.contains("how are you")
            || input_lower.contains("what's up")
            || input_lower.contains("how's it going")
            || input_lower.contains("nice to meet")
            || input_lower.contains("你好吗")
            || input_lower.contains("最近怎么样")
            || input_lower.contains("很高兴认识")
            || input_lower.contains("谢谢")
            || input_lower.contains("你多大了")
            || input_lower.contains("你几岁了")
            || input_lower.contains("你喜欢什么")
            || input_lower.contains("你的爱好")
        {
            return Intent::Chat;
        }
        ...
        // Explicit triggers
        if input_lower.starts_with("research ")
            || input_lower.contains("deep research")
            || input_lower.starts_with("研究 ")
            || input_lower.contains("深度研究")
        {
        ...
        // 4. Analysis
        if input_lower.contains("analyze")
            || input_lower.contains("compare")
            || input_lower.contains("分析")
            || input_lower.contains("比较")
        ...
        // 5. Creative
        if input_lower.contains("write story")
            || input_lower.contains("generate content")
        ...
        // 6. Math
        if input_lower.contains("calculate")
            || input_lower.contains("solve equation")
            || input_lower.contains("math")
        {
            return Intent::Math;
        }
...
mod tests {
    ...
    fn test_classify_intent() {
        ...
        assert_eq!(
            Classifier::classify_intent("Deep research on quantum physics"),
            Intent::DeepResearch
        );
        assert_eq!(
            Classifier::classify_intent("Write a rust function"),
            Intent::Coding
        );
        assert_eq!(
            Classifier::classify_intent("Analyze the stock market"),
            Intent::Analysis
        );
        assert_eq!(
            Classifier::classify_intent("Just a random sentence"),
            Intent::General
        );
    }
    ...
    fn test_assess_complexity() {
        ...
        // Test clamping
        assert!(
            Classifier::assess_complexity(
                "run search analyze compare explain design calculate weather function ```"
            ) <= 1.0
        );
    }
}
