...
impl FusionRouter {
    ...
    async fn build_context(&self, session_id: &str, input: &str) -> Result<FusionRoutingContext> {
        ...
        let memories = self
            ...
            .await
            .map_err(|e| crate::error::CrabletError::Memory(e.to_string()))?;
        let relevant_memories: Vec<String> = memories.into_iter().map(|m| m.content).collect();
    ...
    fn determine_system(&self, context: &FusionRoutingContext) -> SystemRoute {
        if context.complexity_score >= self.config.system3_threshold && self.system3.is_some() {
            SystemRoute::System3
        } else if context.complexity_score >= self.config.system2_threshold
            && self.system2.is_some()
        {
            SystemRoute::System2
        } else {
            SystemRoute::System1
    ...
    fn inject_primary_system_prompt(
        system_prompt: &str,
        mut context_messages: Vec<Message>,
    ) -> Vec<Message> {
        let mut replaced_primary = false;

        for message in &mut context_messages {
            if message.role == "system" && !replaced_primary {
                *message = Message::system(system_prompt.to_string());
                replaced_primary = true;
            }
        }

        if !replaced_primary {
            context_messages.insert(0, Message::system(system_prompt.to_string()));
        }
    ...
    async fn process_with_tools(
        ...
        let _tool_selection_prompt = format!(
            ...
            context.available_tools, context.input
        );
        ...
        if context.input.to_lowercase().contains("weather") {
            if let Ok(result) = self
                .memory
                .invoke_tool("weather", serde_json::json!({"location": "current"}))
                .await
            {
    ...
    async fn process_through_system(
        ...
        let context_messages =
            Self::inject_primary_system_prompt(&context.system_prompt, context_messages);
        ...
        self.memory
            .add_assistant_message(&context.session_id, &response)
            .await
            .map_err(|e| crate::error::CrabletError::Memory(e.to_string()))?;
    ...
    async fn extract_memories(
        &self,
        session_id: &str,
    ) -> std::result::Result<(), crate::error::CrabletError> {
        let session = self
            .memory
...
impl CognitiveSystem for FusionRouter {
    async fn process(&self, input: &str, _context: &[Message]) -> Result<(String, Vec<TraceStep>)> {
        ...
        // Add user message
        self.memory
            .add_user_message(&session_id, input)
            .await
            .map_err(|e| crate::error::CrabletError::Memory(e.to_string()))?;
...
mod tests {
    ...
    #[test]
    fn inject_primary_system_prompt_replaces_only_first_system_message() {
        let context_messages = vec![
            Message::system("Base prompt"),
            Message::system("Conversation summary (Summarize, 3 messages compressed)."),
            Message::user("Continue with the framework work"),
        ];

        let injected = FusionRouter::inject_primary_system_prompt(
            "Enriched prompt with user profile",
            context_messages,
        );

        assert_eq!(injected[0].role, "system");
        assert_eq!(
            injected[0].text().as_deref(),
            Some("Enriched prompt with user profile")
        );
        assert!(injected[1]
            .text()
            .unwrap_or_default()
    ...
    #[test]
    fn inject_primary_system_prompt_inserts_when_missing() {
        let context_messages = vec![Message::user("Need help with routing")];

        let injected = FusionRouter::inject_primary_system_prompt(
            "Enriched prompt with memory",
            context_messages,
        );
