...
impl CognitiveMiddleware for CostGuardMiddleware {
    ...
    async fn execute(
        ...
        if total_tokens > soft_limit {
            ...
            if context.len() > 4 {
                let keep_count = 4;
                let split_idx = if context.len() > keep_count + 1 {
                    context.len() - keep_count
                } else {
                    1
                };
                ...
                if total_tokens > hard_limit {
                    warn!(
                        "CostGuard: Hard limit exceeded ({}). Blocking for compression.",
                        total_tokens
                    );
                    ...
                    let middle_text = middle_messages
                        .iter()
                        .filter_map(|m| m.text())
                        .collect::<Vec<String>>()
                        .join("\n");
                    ...
                    match state.llm.chat_complete(&[summary_msg_req]).await {
                        ...
                        Err(e) => {
                            ...
                            let summary_placeholder = Message::new(
                                "system",
                                "[Context Truncated: Old messages removed for brevity]",
                            );
                    ...
                    *context = new_context;
                    info!(
                        "CostGuard: Context compressed to {} messages",
                        context.len()
                    );
                } else {
                    // SOFT LIMIT: Async compression (background) + Local Truncation
                    info!(
                        "CostGuard: Soft limit exceeded ({}). Triggering background summarization.",
                        total_tokens
                    );
                    ...
                    spawn(async move {
                        ...
                        match llm
                            .chat_complete(&[Message::new("user", &summary_prompt)])
                            .await
                        {
                            Ok(summary) => {
                                event_bus.publish(crate::events::AgentEvent::SystemLog(format!(
                                    "Background Summary Generated: {}",
                                    summary
                                )));
                                // MemoryUpdate event emitted via SystemLog;
                                // specific MemoryUpdate event can be added when
                                // the event architecture supports typed memory events
                            }
