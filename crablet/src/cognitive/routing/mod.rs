...
pub use complexity::{Complexity, ComplexityAnalyzer, TaskCharacteristics};
pub use local::{LocalModelManager, OllamaClient, OllamaConfig};
...
impl RouterStats {
    ...
    pub fn report(&self) -> String {
        ...
        format!(
            ...
            powerful,
            powerful as f64 / total as f64 * 100.0,
            avg_latency
        )
    }
}
    ...
    async fn make_decision(
        ...
        match (complexity, latency_requirement) {
            ...
            (_, LatencyRequirement::Critical) => {
                if self.local_client.is_some() {
                    RouterDecision::Local {
                        ...
                        reason: format!(
                            "Critical latency requirement (<50ms), complexity: {:?}",
                            complexity
                        ),
                    }
                } else {
            ...
            (Complexity::Medium, latency) => {
                match latency {
                    LatencyRequirement::High => RouterDecision::FastModel {
                        model: "gpt-4o-mini".to_string(),
                    ...
                    _ => {
                        ...
                        if fastrand::f32() < 0.3 {
                            RouterDecision::Powerful {
                                ...
                                reason:
                                    "Medium complexity, occasionally use powerful model for quality"
                                        .to_string(),
                            }
                        } else {
                            RouterDecision::FastModel {
                                model: "gpt-4o-mini".to_string(),
                                reason: "Medium complexity, use fast model for cost balance"
                                    .to_string(),
                            }
                        }
                    }
            ...
            // 复杂任务: 使用强力模型
            (Complexity::Complex, _) => RouterDecision::Powerful {
                model: "gpt-4o".to_string(),
                reason: format!(
                    "Complex task requires powerful model, latency: {:?}",
                    latency_requirement
                ),
            },
        }
